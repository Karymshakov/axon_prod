...
def _infer_allowed_tools_from_card(card) -> set[str] | None:
    ...
    if any(marker in title for marker in ('meal', 'питан', 'рацион', 'завтрак', 'ужин')):
        return {'get_room_options', 'get_meal_plan_pricing'}
...
def get_stage_policy(
    ...
    *,
    ...
    if allowed_tools is not None:
        # Photo delivery is read-only and may answer the guest's immediate
        # question without advancing or bypassing the booking stage.
