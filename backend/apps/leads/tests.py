...
class PassiveAiIntakeTests(TestCase):
    def setUp(self):
        ...
        self.org = Organization.objects.create(name='Passive Org', slug='passive-org', owner=self.owner)
        self.owner.current_organization = self.org
        self.owner.save(update_fields=['current_organization'])
        self.config = AIConfig.objects.create(
            organization=self.org,
        ...
        self.lead = Lead.objects.create(
            ...
            telegram_username='passiveguest',
            ai_paused=True,
            notes='Old summary',
        )
        LeadActivity.objects.create(
            ...
            organization=self.org,
            activity_type=LeadActivity.TYPE_TELEGRAM_RECEIVED,
            description='Гость: Нас 4 человека, узнали от друзей. Телефон +996700111222',
            metadata={'text': 'Нас 4 человека, узнали от друзей. Телефон +996700111222'},
        )
    ...
    def test_passive_intake_updates_lead_without_sending_reply(
        self,
        _configured_mock,
        summary_mock,
        extract_mock,
        ...
        summary_mock.return_value = 'Семья из 4 человек хочет забронировать номер'
        extract_mock.return_value = {
            'contact_person': 'Азамат',
            'phone': '+996700111222',
            'guest_count': 4,
        ...
        updated_fields = run_passive_ai_intake(
            self.lead,
            'Нас 4 человека, узнали от друзей. Телефон +996700111222',
            channel='telegram',
        )

        self.lead.refresh_from_db()
        self.assertIn('notes', updated_fields)
        self.assertIn('contact_person', updated_fields)
        self.assertEqual(self.lead.notes, 'Семья из 4 человек хочет забронировать номер')
        self.assertEqual(self.lead.contact_person, 'Азамат')
        self.assertEqual(self.lead.phone, '+996700111222')
    ...
    def test_custom_discovery_source_matches_semantic_guest_phrase(self):
